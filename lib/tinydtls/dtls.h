...
typedef struct {
   ...
   *
   * Used when ServerKeyExchange arrives without a preceding
   * Certificate; the local known server public key is injected.
   *
   * @param ctx     The current dtls context.
   * @param session The session where the key will be used.
...
typedef struct dtls_context_t {
  ...
  dtls_session_ticket_t session_ticket; /**< in-memory RFC5077 ticket cache */
#endif
} dtls_context_t;
...
/**
 * Initializes the single DTLS context supported by this platform-agnostic
 * build. Call only once; returns NULL if already initialized.
 */
dtls_context_t *dtls_new_context(void *app_data);

/** Releases peers for \p ctx. The context storage itself is static. */
