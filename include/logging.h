...
extern "C"
{
    ...
    /*static inline void log_message(int level, const char *category, LogAttributes *attr, void *reserved, const char *fmt, ...) {

    va_list args;
    va_start(args, fmt);

    char buf[256], buf_args[256];
    vsnprintf(buf_args, sizeof(buf_args), fmt, args);
    sprintf(buf, "%s\n", buf_args);
    printf(buf);
...
#define LOG_PRINTF_C(_level, _category, _fmt, ...)                                  \
    ...
    {                                                                               \
        if (LOG_LEVEL_##_level >= LOG_COMPILE_TIME_LEVEL)                           \
        {                                                                           \
            log_printf_a(LOG_LEVEL_##_level, _category, NULL, _fmt, ##__VA_ARGS__); \
        }                                                                           \
    } while (0)
